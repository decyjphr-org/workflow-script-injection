...
In your repository, `click` on the [`Actions`](https://github.com/robandpdx/workflow-script-injection/actions) tab
...
This will take you to the `Actions` page and now click on the `new workflow` button to create a workflow. Alternatively, you can click [this link](https://github.com/robandpdx/workflow-script-injection/actions/new).
...
When the file was committed, it got `pushed` into the repository and the `Actions WorkFlow CodeQL` should be triggered. Now `click` on the [`Actions`](https://github.com/robandpdx/workflow-script-injection/actions) tab and you should see the workflow being scheduled to run based on the `push` event. 
